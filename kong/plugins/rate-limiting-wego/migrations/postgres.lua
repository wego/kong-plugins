return {
  {
    ...
    up = [[
      CREATE TABLE IF NOT EXISTS ratelimiting_metrics_wego(
        api_id uuid,
        identifier text,
        period text,
      ...
      CREATE OR REPLACE FUNCTION increment_rate_limits_wego(a_id uuid, i text, p text, p_date timestamp with time zone, v integer) RETURNS VOID AS $$
      BEGIN
        LOOP
          UPDATE ratelimiting_metrics_wego SET value = value + v WHERE api_id = a_id AND identifier = i AND period = p AND period_date = p_date;
          IF found then
            RETURN;
          END IF;

          BEGIN
            INSERT INTO ratelimiting_metrics_wego(api_id, period, period_date, identifier, value) VALUES(a_id, p, p_date, i, v);
            RETURN;
    ...
    down = [[
      DROP TABLE ratelimiting_metrics_wego;
    ]]
  },
  {
